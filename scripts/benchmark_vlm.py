"""
DriveLM Local VLM Benchmark  —  LLaVA only
===========================================
Benchmarks `llava-hf/llava-1.5-7b-hf` on parsed DriveLM QA data.
...
CAMERA_STRATEGY = {
    ...
    'planning'  : ['CAM_FRONT','CAM_FRONT_LEFT','CAM_FRONT_RIGHT',
                   'CAM_BACK','CAM_BACK_LEFT','CAM_BACK_RIGHT'],
    'behavior'  : ['CAM_FRONT','CAM_FRONT_LEFT','CAM_FRONT_RIGHT',
                   'CAM_BACK','CAM_BACK_LEFT','CAM_BACK_RIGHT'],
}
...
def resolve_image_path(image_path: str, nusc_root: str) -> Path | None:
    ...
    if path.exists():
        return path
    # Remove ../nuscenes/ prefix from path  (e.g., ../nuscenes/samples/CAM_FRONT/file.jpg)
...
def load_pil_image(image_path: str, nusc_root: str,
    ...
    """
    Load image as PIL, resize to max_size on longest edge to save VRAM.
    448px is standard for most VLMs. Use 336 for very tight VRAM.
    """
...
def get_images_for_row(row: pd.Series, category: str,
    ...
    def parse_image_paths(value: str) -> dict:
        paths = {}
        value = str(value)
    ...
    # Use focused evidence for perception/prediction, but full surround context
    # for planning/behavior where the answer depends on global scene state.
    if category in ('planning', 'behavior'):
        ...
        if not filtered:
            ...
            filtered = {cam: all_paths[cam] for cam in strategy_cams if cam in all_paths}
...
def load_model():
    """Load the single supported LLaVA model and processor."""
...
def build_prompt_text(question: str, cam_names: list) -> str:
...
def infer_llava(model, processor, images: dict,
                question: str, max_new_tokens: int = 200) -> str:
    """LLaVA inference."""

    cam_names  = list(images.keys())
    pil_images = list(images.values())
    prompt_text = build_prompt_text(question, cam_names)

    # LLaVA uses one <image> token per image in the prompt
    ...
    # Decode only new tokens (strip input)
...
def run_inference(model, processor, cfg: dict,
                  images: dict, question: str) -> tuple[str, float]:
    """Run LLaVA inference and return (prediction_text, latency_ms)."""
    t0 = time.perf_counter()
    pred = infer_llava(model, processor, images, question)
...
def classify_failure(pred: str, ref: str, category: str, rl: float) -> str | None:
    ...
    cameras_words = {'front','back','left','right'}

    pred_status  = status_words   & set(pred_n.split())
    ref_status   = status_words   & set(ref_n.split())
    pred_cams    = cameras_words  & set(pred_n.split())
    ref_cams     = cameras_words  & set(ref_n.split())
...
def run_benchmark(df: pd.DataFrame, model, processor, cfg: dict,
                  ...
                  max_image_size: int = 448) -> pd.DataFrame:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        # Run inference
        try:
            pred, latency_ms = run_inference(model, processor, cfg, images, question)
        except torch.cuda.OutOfMemoryError:
            torch.cuda.empty_cache()
            gc.collect()
            vram_used = torch.cuda.memory_allocated() / 1024**3 if torch.cuda.is_available() else 0
            skipped += 1
            print(f'\n  ⚠️  OOM on frame {row.get("frame_token","")[:8]} — skipping')
            results.append({
                'scene_token'  : row.get('scene_token', ''),
                'frame_token'  : row.get('frame_token', ''),
                'qa_category'  : category,
...
def generate_report(df: pd.DataFrame, model_key: str,
    ...
    lines += [
        f'  Model      : {cfg["hf_id"]}',
        f'  Quantize   : {cfg.get("quantize") or "none"}',
    ...
    metrics = {
        ...
        'Peak VRAM GB'       : valid['vram_used_gb'].max() if 'vram_used_gb' in valid else 0,
    }
    ...
    # Per question type
    s('METRICS BY QUESTION TYPE (sorted by ROUGE-L asc — hardest first)')
    ...
    fm['description'] = fm['failure_mode'].map(descriptions)
    ...
    for _, r in succ.iterrows():
        ...
        succ_rows.append({'category':r['qa_category'],'rouge_l':r['rouge_l'],
    ...
    for ftype in ['A_hallucination','B_wrong_status','C_wrong_camera',
        ...
        for _, r in valid[valid['failure_mode'] == ftype].head(2).iterrows():
            ...
            fail_rows.append({'failure_mode':ftype,'category':r['qa_category'],
...
def dry_run(df: pd.DataFrame, nusc_root: str, output_dir: str):
    ...
    for _, row in df.iterrows():
        ...
        imgs = get_images_for_row(row, cat, nusc_root, max_size=32)  # tiny load
        n_found = len(imgs)
        n_missing = len(CAMERA_STRATEGY.get(cat, ['CAM_FRONT'])) - n_found
        found   += n_found
        missing += n_missing
        for img in imgs.values(): img.close()
        rows.append({'qa_category': cat, 'n_images_found': n_found,
...
def parse_args():
    ...
    p.add_argument('--images',    required=True,  help='nuScenes root directory')
    ...
    p.add_argument('--img-size',  type=int, default=448,
                   help='Max image dimension (336 for tight VRAM, 448 default, 672 for quality)')
    p.add_argument('--seed',      type=int, default=42)
    p.add_argument('--detect',    action='store_true', help='Show GPU info and exit')
